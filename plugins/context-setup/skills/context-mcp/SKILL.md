...
Server-specific optimization templates are defined in `<plugin_dir>/data/mcp-templates.json` with per-service `last_verified` dates. The templates below are authoritative prompt context; the data file enables independent testing and staleness tracking.
