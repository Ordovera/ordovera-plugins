...
export function buildServerReport(
  ...
  return {
    ...
    flags: {
      ...
      hasActorAttribution: patterns.actorAttribution.length > 0,
...
function formatComparisonTable(servers: ServerReport[]): string[] {
  ...
  lines.push("");
  lines.push(
    "| Server | Write Tools | Auth | Actor Attribution | Logging | Gates | Gaps |"
  );
  lines.push(
    "|--------|-------------|------|-------------------|---------|-------|------|"
  );
  ...
  for (const s of servers) {
    ...
    const auth = s.flags.hasPerToolAuth
      ...
      : s.flags.hasAuth
        ? "global"
        : "none";
    const attribution = s.flags.hasActorAttribution ? "yes" : "no";
    const logging = s.flags.hasLogging ? "yes" : "no";
    const gates = s.flags.hasConfirmationGates ? "yes" : "no";
    const gapCount = s.accountabilityGaps.length;

    lines.push(
      `| ${s.name} | ${writeCount} | ${auth} | ${attribution} | ${logging} | ${gates} | ${gapCount} |`
    );
  }
...
function formatServerSection(server: ServerReport): string[] {
  ...
  lines.push(
    `- [${server.flags.hasActorAttribution ? "X" : " "}] Actor attribution in logs (${server.patterns.actorAttribution?.length ?? 0} occurrences)`
  );
  lines.push(
  ...
  if (!server.flags.hasActorAttribution && server.flags.hasLogging) {
    lines.push(
